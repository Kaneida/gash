...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GASH);
    ...
    if (!settings.contains("nAnonymizeGASHAmount"))
        settings.setValue("nAnonymizeGASHAmount", 1000);

    nAnonymizeGASHAmount = settings.value("nAnonymizeGASHAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGASHAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeGASHAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGASHAmount:
            return QVariant(nAnonymizeGASHAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGASHAmount:
            nAnonymizeGASHAmount = value.toInt();
            settings.setValue("nAnonymizeGASHAmount", nAnonymizeGASHAmount);
            emit anonymizeGASHAmountChanged(nAnonymizeGASHAmount);
            break;
        case CoinControlFeatures:
